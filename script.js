...
try:
    with redirect_stdout(_stdout), redirect_stderr(_stderr):
        try:
            _value = repr(eval(_user_code, {}))
        except SyntaxError:
            exec(_user_code, {})
except Exception:
    _error = traceback.format_exc()

{"stdout": _stdout.getvalue(), "stderr": _stderr.getvalue(), "error": _error, "value": _value}
`);
  ...
  pyodideInstance.globals.delete("_user_code");
...
runButton.addEventListener("click", async () => {
  ...
  try {
    ...
    const combined = [result.stdout?.trim(), result.value?.trim(), result.stderr?.trim(), result.error?.trim()]
      .filter(Boolean)
      .join("\n");
